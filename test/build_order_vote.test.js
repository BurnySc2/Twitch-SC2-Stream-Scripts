...
test("Script executes 'start_vote' and 'update_vote' functions of build_order_vote.js file and changes content of DOM", () => {
    ...
    for (let i = 0; i < content_dict["bos"].length; i++) {
        expect(document.getElementById("bo" + i.toString() + "-name").textContent).toEqual(
            "build order " + (i + 1).toString()
        )
        expect(document.getElementById("bo" + i.toString() + "-percentage").textContent).toEqual(
            "0%"
        )
    ...
    for (let i = 0; i < content_dict["percentages"].length; i++) {
        expect(document.getElementById("bo" + i.toString() + "-percentage").textContent).toEqual(
            content_dict["percentages"][i]
        )
        expect(document.getElementById("bo" + i.toString() + "-bar").style.width).toEqual(
            content_dict["percentages"][i]
        )
    }
